...
namespace Engine
{
	...
	template <class T> bool Cas(Atomic<T>& _x, T _expected, T _value) { return _x.compare_exchange_strong(_expected, _value); }
	...
	{
		assert(_data || !_size);
		const uint8* p = (const uint8*)_data;
		while (_size--)
			_hash = *p++ + (_hash << 6) + (_hash << 16) - _hash;
	...
	inline uint StrHash(const String& _str, uint _hash = 0) { return StrHash(_str.c_str(), _hash); }
	uint NameHash(const char* _str, uint _hash);
	inline uint NameHash(const String& _str, uint _hash = 0) { return NameHash(_str.c_str(), _hash); }
	int StrCompare(const char* _a, const char* _b, bool _ignoreCase = false);
	void StrSplit(const char* _str, const char* _delimiters, Array<String>& _dst);
	inline void StrSplit(const String& _str, const char* _delimiters, Array<String>& _dst) { StrSplit(_str.c_str(), _delimiters, _dst); }
	String StrTrim(const char* _str, const char* _cset, bool _left = true, bool _right = true);
	inline String StrTrim(const String& _str, const char* _cset, bool _left = true, bool _right = true) { return StrTrim(_str.c_str(), _cset, _left, _right); }
	String StrReplace(const char* _str, const char* _from, char _to);
	inline String StrReplace(const String& _str, const char* _from, char _to) { return StrReplace(_str.c_str(), _from, _to); }
	...
	inline bool IsFullPath(const String& _path) { return IsFullPath(_path.c_str()); }
	...
	inline void SplitPath(const String& _path, String* _device, Array<String>* _items) { SplitPath(_path.c_str(), _device, _items); }
	...
	inline String MakeFullPath(const String& _path, const String& _root = "") { return MakeFullPath(_path.c_str(), _root.c_str()); }
	...
	// do define for fast half-float conversion
