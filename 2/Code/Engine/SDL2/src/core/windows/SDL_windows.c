...
WIN_SetErrorFromHRESULT(const char *prefix, HRESULT hr)
{
	...
	if (hr == 0xc0072095) // ERROR_INVALID_VERSION_ARB
	{
		SDL_SetError("%s%s%s", prefix ? prefix : "", prefix ? ": " : "", "Invalid version");
	}
	else if (hr == 0xc0072096) // ERROR_INVALID_PROFILE_ARB
	{
		SDL_SetError("%s%s%s", prefix ? prefix : "", prefix ? ": " : "", "Invalid profile");
	}
	else
	{
		TCHAR buffer[1024];
