...
namespace Rx
{
	TODO_EX("<Containers>", "Постепенно переходить на свою реализацию");
	TODO_EX("<Containers>", "Добавить LockFreeList");
	TODO_EX("String", "Использовать новую реализацию (из RealX на осн. ПК)");
	TODO_EX("Atomic", "добавить memory_order");
	TODO_EX("Atomic", "Решить, использовать ли std::atomic напрямую или делать свою реализацию");
	TODO_EX("Doxygen", "Изменить коментарии с /// на //!");
	TODO_EX("All", "Использовать Atomic и SpinLock напрямую");
	TODO_EX("Name", "Он нужен? Если да, то переместить в другой файл");
	TODO_EX("Config", "Переместить в другой файл");
	TODO_EX("Config", "Рефакторинг");
	...
	//----------------------------------------------------------------------------//
	int8 AtomicAdd(int8& _atomic, int8 _value)
	{
	...
	//----------------------------------------------------------------------------//
	int16 AtomicAdd(int16& _atomic, int16 _value)
	{
	...
	//----------------------------------------------------------------------------//
	int64 AtomicAdd(int64& _atomic, int64 _value)
	{
...
#                   endif
	...
	//----------------------------------------------------------------------------//
	uint32 String::IHash(const char* _str, uint32 _hash)
	{
		if (_str) while (*_str)
			_hash = ToLower(*_str++) + (_hash << 6) + (_hash << 16) - _hash;
		return _hash;
	...
	bool String::Match(const char* _str, const char* _pattern, bool _ignoreCase)
	{
		...
		$_frame:
			...
			if (*_p == '[') // набор символов
			{
				...
				while (*_p != ']')
				{
					if (*_p == '\\' && (_p[1] == '[' || _p[1] == ']' || _p[1] == '\\')) // управляющий символ
						++_p;
					if (*_p == 0)
					{
						--_p;
	...
	int RefCounter::s_numInstances = 0;

	//----------------------------------------------------------------------------//
	RefCounter::RefCounter(RefCounted* _object) :
		m_object(_object),
		m_refs(0),
		m_weakRefs(1)
	{
		AtomicAdd(s_numInstances, 1);
	}
	//----------------------------------------------------------------------------//
	...
	//----------------------------------------------------------------------------//
	void RefCounter::AddRef(void)
	{
		AtomicAdd(m_weakRefs, 1);
	}
	//----------------------------------------------------------------------------//
	void RefCounter::Release(void)
	{
		if (AtomicAdd(m_weakRefs, -1) == 1)
			delete this;
	}
	//----------------------------------------------------------------------------//
	bool RefCounter::Expired(void) const
	{
		return !AtomicGet(m_refs) || !m_object;
	}
	//----------------------------------------------------------------------------//
	RefCounted* RefCounter::_Lock(void)
	{
		ASSERT(AtomicGet(m_weakRefs) > 0);

		int _cnt = AtomicAdd(m_refs, 1); // grab object.
		RefCounted* _object = const_cast<RefCounted*>(m_object);
		if (_cnt > 0 && _object)
			return _object;
		AtomicAdd(m_refs, -1); // no object. restore the counter.
		return nullptr;
	}
	//----------------------------------------------------------------------------//
	int RefCounter::GetNumInstances(void)
	{
		return AtomicGet(s_numInstances);
	}
	//----------------------------------------------------------------------------//
	...
	int RefCounted::s_numInstances = 0;

	//----------------------------------------------------------------------------//
	RefCounted::RefCounted(void)
	{
		AtomicAdd(s_numInstances, 1);
		m_refCounter = new RefCounter(this);
	}
	//----------------------------------------------------------------------------//
	RefCounted::~RefCounted(void)
	{
		ASSERT(m_refCounter == nullptr, "object is destroyed incorrectly");
		AtomicAdd(s_numInstances, -1);
	}
	//----------------------------------------------------------------------------//
	void RefCounted::AddRef(void)
	{
		ASSERT(m_refCounter != nullptr, "object already was destroyed");
		AtomicAdd(m_refCounter->m_refs, 1);
	}
	//----------------------------------------------------------------------------//
	void RefCounted::Release(void)
	{
		ASSERT(m_refCounter != nullptr, "object already was destroyed");
		if (AtomicAdd(m_refCounter->m_refs, -1) == 1)
		{
			m_refCounter->m_object = nullptr;
	...
	//----------------------------------------------------------------------------//
	int RefCounted::GetNumInstances(void)
	{
		return AtomicGet(s_numInstances);
	}
	//----------------------------------------------------------------------------//
